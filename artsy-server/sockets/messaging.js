...
const { verifySocketToken } = require("../utils/socketToken");

const MAX_MESSAGE_LENGTH = 2000;

function registerSocketHandlers(io) {
  // Auth middleware — verifies the short-lived HMAC token issued by /ad-auth/check-auth
  io.use((socket, next) => {
    const token = socket.handshake.auth?.token;
    const userId = verifySocketToken(token);
    if (userId === null) return next(new Error("Invalid or missing auth token"));
